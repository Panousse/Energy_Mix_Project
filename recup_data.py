...
def get_meteo_france_data_from_web(start_year:int, end_year:int) :
    ...
    print(f"Number of files downloaded : {downloaded_files_number} from period {start_year} - {end_year}")
...
def get_energy_data_from_web(region_to_extract:str="Auvergne-Rhône-Alpes", file_output:str="donnees_station_lyon2.csv") :
...
def consolidation_data(energy_input:str='donnees_energie.csv',
    ...
    perfcounterstart = time.perf_counter()
    df1 = pd.read_csv(csv_path+energy_input) # The dataframe from OpenDRÉ
