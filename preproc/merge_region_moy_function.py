...
# On merge et on supprime les colonnes en doublons
    ...
    df_merged.drop(["Altitude","Longitude_y","Latitude_y","Latitude_x","Longitude_x"],axis=1,inplace=True)
# Enfin on fait la moyenne par date et région de chaque station
    df_grouped=df_merged.groupby(["region", "date_timestamp"], as_index=False).mean(numeric_only=True)

    df_grouped.to_csv(PATH_OUTPUT, index=True)
